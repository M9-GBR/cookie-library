...
class Saver {
    set(key = '', value, encode = false) {
        setCookie(key, encode ? btoa(JSON.stringify(value)) : JSON.stringify(value), 365)
    }

    get(key = '', decode = false) {
        return (decode ? JSON.parse(atob(getCookie(key))) : JSON.parse(getCookie(key)))
    }

    exists(key) { return cookieExists(key) }

    getAll = getAllCookies

    clear(key) { clearCookie(key) }

    clearAll = clearAllCookies
}

window.saver = new Saver()
